...
public class SubPartRotate : MonoBehaviour
{
	...
	public int screenDivision = 1000;
	...
	public UnityEvent onRotatePress;
	public UnityEvent onRotateRelease;
	...
	void Update()
	{
		if ((canMove || canRotate) && Input.GetMouseButtonDown(0))
		{
			if (CheckMouseOnSelf())
			{
				...
				isControlling = true;
				currentUnit = 0;
				if (currStatus == SubPartStatus.Moving) { onMovePress.Invoke(); }
				else if (currStatus == SubPartStatus.Rotating) { onRotatePress.Invoke(); }
			}
		}
		...
		if (isControlling)
		{
			if (currStatus == SubPartStatus.Rotating)
			{
				...
				int mouseUnit = Mathf.RoundToInt(e.magnitude / (Screen.height / (float) screenDivision));
				...
				if (unitChange != 0) { onAngleChange.Invoke(); }
